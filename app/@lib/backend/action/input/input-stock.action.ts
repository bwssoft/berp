...
import { inputStockRepository, inputTemporalStockRepository, inputTransactionRepository } from "../../repository/mongodb"
...
export async function updateInputStock() {
  ...
  const stockTemporalAggregate = await inputTransactionRepository.aggregate(stockTemporalPipeline)
  const stockAggregate = await inputTransactionRepository.aggregate(stockPipeline)
...
export async function findAllInputStock() {
  ...
  const aggregate = await inputStockRepository.aggregate(pipeline)
...
export async function getTotalValueInInputStock(input_id: string) {
  const aggregate = await inputStockRepository.aggregate([
    {
      $match: { input_id }
    },
    {
...
export async function analyzeTemporalInputStock(input_id: string) {
  ...
  const pipeline = await inputTemporalStockRepository.aggregate<{
    input: IInput;
    stocks: {
      date: {
        day: number;

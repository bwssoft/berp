...
const _productStockRepository = productStockRepository
const _productTemporalStockRepository = productTemporalStockRepository

export async function updateProductStock() {
  const stockTemporalPipeline = updateStockTemporalAggregate()
  const stockPipeline = updateStockAggregate()
  const stockTemporalAggregate = await _productTransactionRepository.aggregate(stockTemporalPipeline)
  const stockAggregate = await _productTransactionRepository.aggregate(stockPipeline)
...
export async function findAllProductStock() {
  ...
  const aggregate = await _productStockRepository.aggregate(pipeline)
...
export async function getTotalValueInProductStock(product_id: string) {
  const aggregate = await _productStockRepository.aggregate([
    {
      $match: { product_id }
    },
    {
...
export async function analyzeTemporalProductStock(product_id: string) {
  ...
  const pipeline = await _productTemporalStockRepository.aggregate<{
    product: IProduct;
    stocks: {
      date: {
        day: number;

...
const commonColumns: ColumnDef<Row>[] = [
  {
    ...
    accessorKey: "checked",
    cell: ({ row }) => {
      const s = String(row.original.checked) as keyof typeof statuses;
      return (
  ...
  {
    header: "Data da checagem",
    accessorKey: "created_at",
    cell: ({ row }) => <p>{row.original.checked_at.toLocaleString()}</p>,
  },
];
...
export function getColumns(type: Device.Model): ColumnDef<Row>[] {
  return [
    ...
    ...(columnMap[type] || []),
    {
      ...
      cell: ({ row }) => {
        const { original } = row;
        const href = original.configurationLogId
          ? `/production/log/configurator/${original.configurationLogId}`
          : `/engineer/configuration-profile/form/update?id=${original.configurationProfileId}`;
        const title = original.configurationLogId
          ? "Checar log de configuração"
          : "Checar perfil de configuração";
        return (

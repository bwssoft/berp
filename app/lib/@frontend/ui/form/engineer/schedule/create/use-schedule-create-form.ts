import { createManySchedule } from "@/backend/action/engineer/command/schedule.action";
import { findManyDeviceBySerial } from "@/backend/action/engineer/device.action";
...
export function useScheduleCreateForm() {
  ...
  const handleFormatDeviceFromFile = (obj: { Serial: string }) => {
    return {
  ...
  const availableVariables = findByRegex(commandData, /{{.+?}}/g).reduce(
    (acc, cur) => {
      const key = `var_${cur}`;
  ...
  const variablesToBeInserted = findByRegex(commandData, /{{.+?}}/g)
    .filter((v) => {
      return (
        `var_${v}` !== formValues.automaticFillVariables?.firmware &&
        `var_${v}` !== formValues.automaticFillVariables?.serial
      );
    })
    .reduce((acc, cur) => {
      const key = `var_${cur}`;
  ...
  const createCommandPreview = (
    ...
    const commandPreview = replaceVariables(commandData, variables);
    return commandPreview;
  };
  const preview = fields.map(({ serial }) =>
    createCommandPreview(serial, formValues, watchedVariablesToBeInserted)
  );
  ...
  const handleSubmit = hookFormSubmit(async (data) => {
    try {
      ...
      const devicesInDb = await findManyDeviceBySerial(
        devicesInForm.map(({ serial }) => serial)
      );

      if (
        !devicesInForm.every(({ serial }) =>
          devicesInDb.find((d) => d.equipment.serial === serial)
        )
      ) {
        setError("devices", {
          message:
            "Alguns dispositivos não estão cadastrados no banco de dados",
        });
        return;
      }
      ...
      const schedules = devicesInDb.map((d) => ({
        ...
        data: createCommandPreview(
          d.equipment.serial,
          data,
          data.variablesToBeInserted
        ),
        pending: true,

...
class UpdateOneConfigurationProfileUsecase {
  ...
  async execute(query: { id: string }, value: Omit<IConfigurationProfile, "id" | "created_at" | "user_id">) {
    return await this.repository.updateOne(query, { $set: value })
  }
}

export const updateOneConfigurationProfileUsecase = singleton(UpdateOneConfigurationProfileUsecase)

...
class CreateOneConfigurationProfileUsecase {
  ...
  async execute(input: Omit<IConfigurationProfile, "id" | "created_at" | "user_id">) {
    const configuration_profile = Object.assign(input, {
      created_at: new Date(),
      id: crypto.randomUUID(),
      user_id: crypto.randomUUID(),
    })
    return await this.repository.create(configuration_profile)
  }
}

export const createOneConfigurationProfileUsecase = singleton(CreateOneConfigurationProfileUsecase)

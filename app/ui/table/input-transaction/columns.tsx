...
import { IInput, IInputTransaction } from "@/app/lib/definition";
...
export const columns: ColumnDef<IInputTransaction & { input: IInput }>[] = [
  {
    header: "Movimento",
    accessorKey: "type",
    cell: ({ row }) => {
      ...
      return (
        ...
        >
          <CurrentIcon className={cn("h-6 w-5 flex-none")} aria-hidden="true" />
  ...
  },
  {
    ...
    accessorKey: "input",
    cell: ({ row }) => {
      const inputTransaction = row.original;
      return inputTransaction.input.name;
    },
  },
  {
    header: "Quantidade",
    ...
    cell: ({ row }) => {
      const inputTransaction = row.original;
      return inputTransaction.quantity;
    },
  },
  {
